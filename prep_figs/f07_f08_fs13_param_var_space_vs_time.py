...
def collect_mad_params(res_path_syr, res_path_per_site, model_name):
    """
    ...
    param_val_per_site_as_avg_param_dict = {}
    for param in param_list:
        param_val_per_site_as_avg_param_dict[param] = []
        ...
        for param in param_list:
            try:
                ...
                if len(good_yr_keys) >= 3:
                    ...
                    param_val_per_site_as_avg_param_dict[param].append(
                    ...
                    # # using variance
                    # mad_temporal_val_per_site = np.var(np.array(param_val_list))
                    # mad_temporal[param].append(mad_temporal_val_per_site)
    ...
    for param, val in param_val_per_site_dict.items():
        ...
        # # using variance
        # mad_sp = np.var(param_site)
        # mad_sp_site[param] = mad_sp

        # using MAD
        param_site_as_avg_param = np.array(param_val_per_site_as_avg_param_dict[param])
        param_site_as_avg_param_median = np.median(param_site_as_avg_param)
        mad_sp_as_avg_param = np.sum(
            np.absolute(param_site_as_avg_param - param_site_as_avg_param_median)
        ) / len(param_site_as_avg_param)
        mad_sp_site_yr[param] = mad_sp_as_avg_param

        # # using variance
        # param_site_as_avg_param = np.array(param_val_per_site_as_avg_param_dict[param])
        # mad_sp_as_avg_param = np.var(param_site_as_avg_param)
        # mad_sp_site_yr[param] = mad_sp_as_avg_param

    for param, list_arr in mad_temporal.items():
        mad_temporal[param] = np.array(list_arr)

    return mad_sp_site, mad_sp_site_yr, mad_temporal
...
def plot_axs_param_dens(
    ...
    perc_diff_mad_tp_mad_sp_site_yr = percentage_difference(
        np.median(mad_temporal), mad_sp_site_yr_val
    )

    return perc_diff_mad_tp_mad_sp_site, perc_diff_mad_tp_mad_sp_site_yr


def plot_param_variance(per_site_yr_res_path, per_site_res_path, model_name):
    """
    ...
    mad_sp_site_dict, mad_sp_site_yr_dict, mad_temporal_dict = collect_mad_params(
    ...
    if model_name == "LUE_model":
        ...
        for plot_ix, p_to_plot in enumerate(param_list_lue_clim):
            ...
            perc_diff_tp_sp_site, perc_diff_tp_sp_site_yr = plot_axs_param_dens(
                axs[ax_x, ax_y],
                mad_sp_site_dict[p_to_plot],
                mad_sp_site_yr_dict[p_to_plot],
        ...
        plt.savefig(
            "./figures/f07_param_clim_variance_lue.png", dpi=300, bbox_inches="tight"
        )
        plt.savefig(
            "./figures/f07_param_clim_variance_lue.pdf",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close("all")
        ...
        os.makedirs(fig_path, exist_ok=True)
        plt.savefig(
            "./figures/f08_param_hydro_variance_lue.png", dpi=300, bbox_inches="tight"
        )
        plt.savefig(
            "./figures/f08_param_hydro_variance_lue.pdf",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close("all")
    ...
    # prepare the figure for P model
        ...
        os.makedirs(fig_path, exist_ok=True)
        plt.savefig(
            "./supplement_figs/fs13_param_variance_p.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.savefig(
            "./supplement_figs/fs13_param_variance_p.pdf",
            dpi=300,
            bbox_inches="tight",
        )
