...
def filter_lue_data(
    ...
    if site_year is None:  # all year optimization
        ...
        # et_sim = lue_model_op["wai_results"][et_var_name][~drop_et_data_indices]
...
def lue_model_cost_function(
    ...
    param_group_to_vary=None,
    param_names_to_vary=None,
    et_var_name="ET",
    unique_yrs_arr=None,
):
    """
    ...
    if (gpp_obs.size == 0) or (
        ...
        if cost_func == "cost_nnse_unc":
            ...
            # calculate average of parameter values per year (for the
            # parameters which will vary per year).

            if (  # skip the parameter groups which are
                # not used in calculating cost_ideal and cost_non_ideal
                (param_group_to_vary != "Group7")
                or (param_group_to_vary != "Group5")
                or (param_group_to_vary != "Group1")
            ):
                for param in param_names_to_vary:
                    if (param == "alpha_fT_Horn") and (
                        ip_df_dict["KG"][0] not in ["C", "D", "E"]
                    ):
                        pass
                    elif (param == "alpha") and (ip_df_dict["KG"][0] != "B"):
                        pass
                    else:
                        temp_par_val = []
                        for yr in unique_yrs_arr:
                            temp_par_val.append(updated_params[f"{param}_{int(yr)}"])
                        updated_params[param] = np.mean(temp_par_val)
