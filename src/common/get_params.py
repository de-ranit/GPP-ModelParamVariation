...
def get_params(ip_df_dict, p_list=None, p_vec=None):
    ...
    params["kco"] = (
        2.09476e5  # O2 partial pressure with standard Atmosphere, i.e., kPo (Pa)
    )
    ...
    params["EaGamma"] = (
        37830.0  # Activation energy for photorespiratory compensation (J mol-1)
    )
    ...
    if (p_list is not None) and (p_vec is not None):
        for pn, pv in enumerate(p_list):
            ...
            if pv not in params:
                param_name_no_yr, _ = pv.rsplit("_", 1)
            else:
                param_name_no_yr = pv
            ...
            if (
                (param_name_no_yr == "alpha")
                and isinstance(params[param_name_no_yr], float)
                or (param_name_no_yr == "alpha_fT_Horn")
                and isinstance(params[param_name_no_yr], float)
            ):
                params[pv] = params[param_name_no_yr]
            else:
                p_value = p_vec[pn] * params[param_name_no_yr]["ini"]
                params[pv] = p_value
